...
# Compile the C++ fibonacci program
subprocess.run(["clang++", "bench_c++.cpp", "-o", "fib_cpp.out"], check=True)
...
plt.title("Fibonacci Execution Time (n = 30 to 40)")
plt.xlabel("n")
plt.ylabel("Time (seconds)")
plt.grid(True, which="both", ls="--", alpha=0.5)
plt.legend()
plt.savefig("fib_benchmark.png", dpi=300)
plt.clf()
...
plt.title("Fibonacci Initial Execution Time")
plt.xlabel("n")
plt.ylabel("Time (seconds)")
plt.grid(True, which="both", ls="--", alpha=0.5)
plt.legend()
plt.savefig("fib_benchmark_init.png", dpi=300)
