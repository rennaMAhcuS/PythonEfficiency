...
subprocess.run(["clang++", "bench_c++.cpp", "-o", "matmul_cpp.out"], check=True)
subprocess.run(["clang++", "-O3", "bench_c++.cpp", "-o", "matmul_opt_cpp.out"], check=True)
...
plt.legend()
plt.savefig("matmul_benchmark.png", dpi=300)
plt.clf()
...
plt.title("Matrix Multiplication Initial Execution Time")
plt.xlabel("n")
plt.ylabel("Time (seconds)")
plt.grid(True, which="both", ls="--", alpha=0.5)
plt.legend()
plt.savefig("matmul_benchmark_init.png", dpi=300)
